...
class DetailViewModelTest {
    ...
    fun downloadCity_WithResult(){
        val cityId = "1"
        Mockito.`when`(networkClient.getCityWeatherById(cityId)).thenReturn(Single.just(Mockito.mock(Response::class.java)))
    ...
    fun downloadCity_WithError(){
        Mockito.`when`(networkClient.getCityWeatherById("1")).thenReturn(Single.error(Exception()))
        viewModel.downloadCity("1")
        Assert.assertEquals(DetailScreenState.Companion.LOADING, viewModel.getStateHolderObservable().value?.getAction())
        Thread.sleep(200)
